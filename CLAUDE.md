...
- **`LabaRugiService.build({ periodId, ytd, projectId?, cabangId? })`**: agregasi 5 kind (PENDAPATAN, BEBAN_POKOK, BEBAN, PENDAPATAN_LAIN, BEBAN_LAIN). Layout: Pendapatan − HPP = Laba Kotor; − Beban Operasi = Laba Usaha; ± Lain-lain = Laba Sebelum Pajak; − PPh = Laba Bersih. Mode `ytd=true` agregasi dari awal tahun buku. `projectId` (via `aggregateAllAccounts`): id spesifik = hanya baris proyek itu, `null` = tanpa proyek, undefined = semua.
- **`LabaRugiProyekService.build({ periodId, ytd, industriId? })`**: batch — loop tiap proyek AKTIF/SELESAI, panggil `LabaRugiService.build({ projectId })`, kembalikan ringkasan per-proyek (pendapatan/HPP/beban operasi/laba bersih/margin) + `detail` (`LabaRugiResponse` lengkap per akun) + total semua proyek. Web `/laporan/laba-rugi-proyek`: tabel ringkasan + filter **Detail akun** (ringkasan saja / satu proyek / **semua proyek bulk**) merender `detail` per-akun tanpa fetch tambahan; PDF "Cetak Semua Proyek" via `/proxy/reports/laba-rugi-proyek.pdf`.
...
- **`ArusKasService.build({ periodId })` — METODE TIDAK LANGSUNG, CAKUPAN PENUH (balance by construction)**:
  - `rawDelta(acc) = kredit − debit` mutasi periode. Untuk akun non-kas = kontribusi bersih ke kas; untuk akun L/R Σ = laba bersih. Karena tiap jurnal Σdebit=Σkredit, **Σ rawDelta seluruh akun non-kas = Δkas** → operasi+investasi+pendanaan **selalu** = Δkas.
  - Tiap akun non-kas dipartisi via `klasifikasiNeraca`: **Operasi** = akun L/R (laba bersih) + aset lancar non-kas + liabilitas jangka pendek + kontra-aset (akum. penyusutan/penyisihan = add-back); **Investasi** = aset tetap bruto (normalBalance DEBIT, klasifikasi TETAP); **Pendanaan** = liabilitas jangka panjang + SEMUA ekuitas. Baris terkenal di-itemisasi + baris "Lainnya" (residual) menjaga jumlah baris = total bucket.
  - **Tidak ada lagi daftar akun hardcoded** (dulu hanya 1-103..1-107 / 2-101 dst → akun lancar baru mis. 1-108 Piutang Intercompany bocor & bikin tak balance). Akun baru otomatis tercakup lewat klasifikasi.
  - Validasi: `kasAwal + kenaikanBersih ≈ kasAkhir` (toleransi 0.5 rupiah) — kini selalu balanced.
- **`PerubahanEkuitasService.build({ periodId })`**: "Modal" = **SEMUA akun ekuitas KECUALI Saldo Laba (3-102) & Dividen (3-104)** — mencakup Modal Disetor (3-101), Tambahan Modal Disetor/agio (3-106), Ikhtisar Laba Rugi (3-103), dst. + laba bersih periode − dividen = saldo akhir. Digeneralisasi supaya total ekuitas **konsisten dengan Neraca** (dulu hanya 3-101 → selisih senyap bila ada 3-106 dll).
- **Klasifikasi laporan = DATA di Account, bukan prefix kode** (sejak migrasi `add_klasifikasi_neraca`):
  ...
  - Arus Kas & Perubahan Ekuitas kini **digeneralisasi berbasis klasifikasi/kind** (bukan daftar kode) — lihat entri masing-masing di atas. Baris "Lainnya" (residual) menampung akun non-standar sehingga tak ada mutasi yang bocor tanpa terkategorikan.
...
- **Engine** = FUNGSI MURNI `computeConsolidation()` di **`consolidation.engine.ts`** (tanpa Prisma, deterministik, **ter-unit-test** `__tests__/consolidation.engine.test.ts`, `pnpm exec vitest run consolidation.engine`). `consolidation.service.ts` hanya baca balances per entitas (RLS) → panggil engine. Gabung 100% tiap entitas per **kode akun** → eliminasi akun ber-flag intercompany → NCI = Σ (minoritas% × aset bersih anak). Neraca s/d endDate, Laba Rugi rentang. `ekuitasInduk = totalEkuitasKonsolidasi − NCI`.
- **Kertas kerja per-entitas**: tiap baris akun bawa `perEntity {tenantId→nilai}` → worksheet web menampilkan kolom PER-ENTITAS → Gabungan → Eliminasi → Konsolidasi (Kode/Akun sticky).
- **Blok `integritas`** (sinyal validitas): `neracaBalanced`/`selisihNeraca` (CATATAN: strukturnya ~0 karena `totalEkuitasKons` diturunkan `Aset−Liab` — near-tautologi), **sinyal SEJATI = rekonsiliasi intercompany** (`icTerekonsiliasi`, `totalSelisihIntercompany`, `jumlahIcTidakCocok`), `entitasBelumTutupBuku` (baca `FiscalPeriod.status` per entitas: OPEN/CLOSING), `entitasTanpaData`. Web tampilkan **peringatan keras** (banner) tapi laporan tetap tampil.
- **Audit trail**: `consolidate()` menulis `AuditLog` (action **`GENERATE`** — enum baru) siapa/kapan/parameter, best-effort (gagal audit tak menggagalkan laporan). Model `AuditLog` sebelumnya dorman; ini penulis pertamanya.
- **Ekspor** `consolidation-export.service.ts` (exceljs/pdfmake, tanpa lib baru): `GET /consolidation/report.{xlsx,pdf}` → web tombol PDF/Excel via `/proxy/consolidation/report.*`. Excel: Ringkasan/KPI+integritas+entitas, Neraca kertas-kerja per-entitas, Laba Rugi, Rekonsiliasi IC. **Grafik** kontribusi entitas = SVG inline (tanpa chart lib). Single-currency IDR (tak ada translasi mata uang).
