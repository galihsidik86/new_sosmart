...
const createSchema = z.object({
  ...
  hari: z.coerce.number().int().min(0).max(365),
  urutan: z.coerce.number().int().min(0).max(999).optional(),
});
type CreateInput = z.infer<typeof createSchema>;

const updateSchema = z.object({
  nama: z.string().min(1).max(60).optional(),
  hari: z.coerce.number().int().min(0).max(365).optional(),
  aktif: z.boolean().optional(),
  urutan: z.coerce.number().int().min(0).max(999).optional(),
});
type UpdateInput = z.infer<typeof updateSchema>;

@Controller('term-pembayaran')
@UseGuards(TenantGuard, RolesGuard)
@UseInterceptors(TenancyInterceptor)
export class TermPembayaranController {
  constructor(private readonly svc: TermPembayaranService) {}
