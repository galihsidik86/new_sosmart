  ...
  return (
    <form action={formAction} className="space-y-3 text-sm">
      ...
      <FormField label="Kode" required>
        <Input name="kode" required defaultValue={d.kode ?? ''} invalid={!!fe.kode} placeholder="PLG-006" />
        <FieldError msg={fe.kode} />
      </FormField>
      <FormField label="Nama" required>
        <Input name="nama" required defaultValue={d.nama ?? ''} invalid={!!fe.nama} placeholder="PT / CV …" />
      ...
      <FormField label="Jenis Pelanggan">
        <Select name="jenisPelangganId" defaultValue={d.jenisPelangganId ?? ''}>
      ...
      <div className="grid grid-cols-2 gap-2">
        <FormField label="Termin (hari)">
          <Input name="terminHari" type="number" defaultValue={String(d.terminHari ?? 14)} invalid={!!fe.terminHari} />
          <FieldError msg={fe.terminHari} />
        </FormField>
        <FormField label="Limit kredit">
          <Input name="kreditLimit" type="number" defaultValue={d.kreditLimit ?? '0'} invalid={!!fe.kreditLimit} />
