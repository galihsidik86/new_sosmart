...
async function createJenis(formData: FormData) {
  ...
  revalidatePath('/master/jenis-projek');
}
...
async function updateJenis(formData: FormData) {
  ...
  await apiFetch(`/jenis-projek/${id}`, {
...
async function deleteJenis(formData: FormData) {
  ...
  if (!tenantId) redirect('/login');
  const id = String(formData.get('id'));
  await apiFetch(`/jenis-projek/${id}`, { method: 'DELETE', tenantId });
  revalidatePath('/master/jenis-projek');
}

export default async function JenisProjekPage() {
  await getSession();
  const tenantId = (await getActiveTenantId())!;
  const list = await apiFetch<Jenis[]>('/jenis-projek?includeInactive=true', { tenantId });

  return (
    <PageContainer size="list">
      <PageHeader
        title="Jenis Projek"
        subtitle="Kategori projek yang bisa Anda atur sendiri per perusahaan (mis. Consumer Based, Brand Based, Personal Based). Dipilih di form projek & dipakai filter laporan proyek."
      />
            ...
            <TBody>
              {list.map((t) => (
                <TR key={t.id} className={t.aktif ? '' : 'text-tanah-500 bg-cream-50/40'}>
                  ...
                  <MoneyCell className="text-tanah-500">{t._count?.projects ?? 0}</MoneyCell>
                  <TD className="text-center">
                    {t.aktif ? <Badge variant="success" size="sm">Aktif</Badge> : <Badge variant="neutral" size="sm">Non</Badge>}
                  </TD>
                  <TD className="text-right">
                    <form action={deleteJenis} className="inline">
                      <input type="hidden" name="id" value={t.id} />
                      <button className="text-xs text-bata-500 hover:underline font-semibold" type="submit">hapus</button>
                    </form>
                  </TD>
                </TR>
              ))}
              {list.length === 0 && (
                <EmptyRow colSpan={4}>Belum ada jenis projek. Tambah di panel kanan (mis. Consumer Based).</EmptyRow>
              )}
            </TBody>
          </Table>
        </section>
