...
async function postSalesAction(id: string): Promise<PostResult> {
  'use server';
  ...
  try {
    ...
    return { ok: false, error: e instanceof Error ? e.message : 'Gagal post faktur' };
  }
}

async function overrideSalesPostAction(
  id: string,
  formData: FormData,
  ...
  const r = await runWithApprover({
    ...
    apiPath: `/sales-invoices/${id}/post`,
...
async function cancelWithApproverAction(formData: FormData): Promise<{ error?: string } | void> {
  ...
  const r = await runWithApprover({
    ...
    apiPath: `/sales-invoices/${id}/cancel`,
    body: { alasan },
    requestedByUserId: session?.user.id,
  });
  if (!r.ok) return { error: r.error };
  ...
  return (
    <>
      <PageContainer size="form">
        ...
        {inv.status === 'DRAFT' && (
          <div className="mb-3">
            <ApprovalPanel docType="PENJUALAN" docId={inv.id} path={`/transaksi/penjualan/${inv.id}`} />
          </div>
        )}
        ...
        <div className="flex items-center gap-3">
          ...
          {inv.status === 'DRAFT' && (
            <>
              <PostButton
                label={mayPost ? 'Post Faktur (terbitkan jurnal)' : 'Post (perlu approval Akuntan)'}
                requiresStepUpFirst={!mayPost}
                postAction={postSalesAction.bind(null, inv.id)}
                overrideAction={overrideSalesPostAction.bind(null, inv.id)}
              />
              <Link
          ...
          {(inv.status === 'POSTED' || inv.status === 'PARTIAL') && !mayCancel && s.role === 'AKUNTAN' && (
            <StepUpButton
              ...
              description="Pembatalan akan reverse jurnal & HPP. Masukkan alasan dan kredensial owner/admin."
              variant="danger"
