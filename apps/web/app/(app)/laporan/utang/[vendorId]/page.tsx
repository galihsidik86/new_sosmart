...
export default async function UtangStatementPage({
  ...
  return (
    <>
      <Topbar breadcrumb={`Laporan / Utang / ${st.vendor.kode}`} tenantNama={s.tenantNama!} />
        ...
        <div className="mb-6 flex items-start justify-between">
          <div>
            ...
            <p className="text-sm text-tanah-500 mt-1">
              Kode {st.vendor.kode} · patokan {fmtTanggal(asOf)}
            </p>
          </div>
          <div className="flex items-start gap-4">
            <div className="flex flex-col gap-2">
              <a
                href={`/proxy/reports/ap-statement.xlsx?vendorId=${vendorId}&asOf=${asOf}${cabangId ? `&cabangId=${cabangId}` : ''}`}
              ...
              <a
                href={`/proxy/reports/ap-statement.pdf?vendorId=${vendorId}&asOf=${asOf}${cabangId ? `&cabangId=${cabangId}` : ''}`}
                target="_blank"
